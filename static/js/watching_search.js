// Búsqueda de Mi TV, calcada de books_search.js: el botón alterna entre
// abrir el modal y limpiar el filtro activo, y delega en watchingApplySearch.
document.addEventListener('DOMContentLoaded', function () {
    const searchBtn = document.getElementById('watchSearchBtn');
    const searchModal = document.getElementById('watchSearchModal');
    const searchForm = document.getElementById('watchSearchForm');
    const searchInput = document.getElementById('watchSearchInput');
    const searchCloseBtn = document.getElementById('watchSearchCloseBtn');
    const searchCancelBtn = document.getElementById('watchSearchCancelBtn');
    if (!searchBtn) {
        return;
    }
    ...
    const clearFilter = () => {
        currentQuery = '';
        if (typeof window.watchingApplySearch === 'function') {
            window.watchingApplySearch('');
        }
        if (searchInput) {
            searchInput.value = '';
        }
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', function (event) {
            ...
            if (typeof window.watchingApplySearch === 'function') {
                window.watchingApplySearch(currentQuery)
                    .then((data) => {
                        closeModal();
                        updateButtonState(data.total_watched);
                    })
                    .catch(() => {
                        closeModal();
                    });
